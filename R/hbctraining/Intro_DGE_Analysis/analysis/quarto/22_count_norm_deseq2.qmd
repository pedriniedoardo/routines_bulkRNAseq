...
![norm](img/de_workflow_salmon_normalization.png){width="400"}
:::
...
::: {#fig-norm2 fig-align="center"}
![norm2](img/normalization_methods_depth.png){width="400"}
:::
...
::: {#fig-norm3 fig-align="center"}
![norm3](img/normalization_methods_length.png){width="200"}
:::
...
::: {#fig-norm4 fig-align="center"}
![norm4](img/normalization_methods_composition.png){width="400"}
:::
...
# read in the data form the previous process
...
# are all the sample in the count table alo in the meatdata
all(colnames(txi$counts) %in% rownames(meta))

# are the sample in the table and in the metadata with the same orider?
...
```{r}
# simulate the scrambling
test <- txi$counts[,sample(colnames(txi$counts))]

# confirm the scrambling is happening
all(colnames(test) %in% rownames(meta))
all(colnames(test) == rownames(meta))

# rearrange the sample in the count matrix to match the order in the metadata
test_fix <- test[,rownames(meta)]

# confirm the scrambling is fixed
all(colnames(test_fix) %in% rownames(meta))
all(colnames(test_fix) == rownames(meta))
```

Here the solution provided

```{r}

idx <- match(rownames(meta), colnames(test))
idx

data_reordered <- test[, idx]

## OR

data_reordered <- test[, match(rownames(meta), colnames(test))]
...
```{r}
# confirm the structure of the txi obejct
str(txi)

## Create DESeq2Dataset object
dds <- DESeqDataSetFromTximport(txi, colData = meta, design = ~ sampletype)

# now confirm that after loading, the data have effectively been rounded to integers
dds@assays@data$counts[1:10,]
```
...
```{r}
# read in the matrix of rounded counts generated from txi
raw_count <- readRDS("../../out/object/raw_count.rds")
raw_count[1:10,]

# generate the deseq2 object
dds_alt <- DESeqDataSetFromMatrix(raw_count, colData = meta, design = ~ sampletype)

# now confirm the identity of the matrix
dds_alt@assays@data$counts[1:10,]
```

::: {#fig-deseq1 fig-align="center"}
![deseq1](img/deseq_obj1.png){width="300"}
:::
...
![salmon](img/de_workflow_salmon_normalization.png){width="300"}
:::
...
```{r}
write.table(normalized_counts, file="../../out/table/normalized_counts.txt", sep="\t", quote=F, col.names=NA)
```
...
Save the object for the next steps

```{r}
saveRDS(dds,"../../out/object/dds.rds")
```
