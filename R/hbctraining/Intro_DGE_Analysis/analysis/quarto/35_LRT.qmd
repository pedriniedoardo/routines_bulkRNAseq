...
```{r}
# read in the data.
# this is after running after runnign DESeq() function
...
dds <- DESeqDataSetFromMatrix(raw_count, colData = meta, design = ~ sampletype)
...
The result above was produced using a reduced formula rather than a design matrix. Try to generate the same object but using a reduce design matrix rather than a formula.

```{r}
# The full model includes the variable of interest ('sampletype')
full_design <- model.matrix(~ sampletype, data = meta)

# print the model.matrix 
full_design

# build the alternative dds using the design as a matrix
dds_alt <- DESeqDataSetFromMatrix(raw_count, colData = meta, design = full_design)

# Run analysis
dds_alt <- DESeq(dds_alt)

# The reduced model only includes the intercept. This is the matrix equivalent of the formula '~ 1'.
reduced_design <- model.matrix(~ 1, data = meta)

# print the model.matrtix
reduced_design

# Likelihood ratio test
dds_alt_lrt <- DESeq(dds_alt, test="LRT", reduced = reduced_design)

# save the lrt object
saveRDS(dds_alt_lrt,"../../out/object/dds_alt_lrt.rds")

# Extract results for LRT
res_alt_LRT <- results(dds_alt_lrt)

```

Confirm that the results produced from the formula or the model.matrix are the same.

```{r}
identical(data.frame(res_LRT),
          data.frame(res_alt_LRT))

all.equal(data.frame(res_LRT),
          data.frame(res_alt_LRT))
```
...
intersect(sigLRT_genes$gene,rownames(sigOE)) %>% length()
...
```{r}
# test the intersection
intersect(sigLRT_genes$gene,rownames(sigKO)) %>% length()
...
```{r}
# generate the rlog values from the main object
# rld <- rlog(dds, blind=TRUE)
rld <- rlog(dds_lrt, blind=TRUE)
rld_mat <- assay(rld)

# identical(rld_mat,assay(rlog(dds, blind=TRUE)))
head(rld_mat)
```
